...
SERVER_ONE_PATH=$BASE_PATH/user

SERVER_ONE_JAR=user-one-spring.jar

ORIGIN_JAR_NAME=app-user-0.0.1-SNAPSHOT.jar
...
RES_CODE=$(curl -o /dev/null -w "%{http_code}" "http://localhost:8080/login")
if [ $RES_CODE -eq 200 ];
then
  echo "OKOK"
  TARGET_ONE="live"
